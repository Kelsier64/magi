...
3. **Permanent Storage (LTM)**: If you synthesize a highly critical fact or standard that must be remembered permanently, contact `LTM-Manager` via the `send_message` tool. 
    - You can ask `LTM-Manager` to **create** new memories.
    - You can ask `LTM-Manager` to **edit** or **remove** existing memories.
    - You can ask `LTM-Manager` to **activate** or **deactivate** specific memories for your current tasks.